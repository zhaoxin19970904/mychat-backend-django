...
from .models import User,Course,UserCourse,Chat,Message
...
def chatmessage(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'id': chat.id, 'name': chat.name, 'message': chat.message}, status=201)
    return JsonResponse({'error': 'Method not allowed'}, status=405)


@csrf_exempt
def getchat(request, cid):  # Now expecting 'cid' as an argument
    if request.method == 'GET':
        chats = Chat.objects.filter(cid=cid, isDeleted=False).order_by('-created_at')
        chats_list = list(chats.values('id', 'name', 'message', 'created_at'))
        return JsonResponse({'chats': chats_list}, safe=False, status=200)
    return JsonResponse({'error': 'Method not allowed'}, status=405)

@csrf_exempt
def chatdetails(request, id):  # Now expecting 'cid' as an argument
    if request.method == 'GET':
        chats = Chat.objects.filter(id=id, isDeleted=False).order_by('-created_at')
        chats_list = list(chats.values('id', 'name', 'message', 'created_at'))
        print(chats_list)
        return JsonResponse({'chats': chats_list}, safe=False, status=200)
    return JsonResponse({'error': 'Method not allowed'}, status=405)
...
def clients(request):
    try:
        pageSize = int(request.GET.get('pageSize', 2))
        page = int(request.GET.get('page', 1))
        user = User.objects.filter(type='client', isDeleted=False)
        totalItems = user.count()
        totalPages = totalItems // pageSize + (1 if totalItems % pageSize > 0 else 0)
        if page > totalPages:
            return JsonResponse({'error': 'Page not found'}, status=404)
...
def get_clients(request):
    try:
        clients = User.objects.filter(type='client', isDeleted=False)
